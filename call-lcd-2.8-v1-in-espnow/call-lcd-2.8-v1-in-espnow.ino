...
void onEspNowSent(const uint8_t* mac_addr, esp_now_send_status_t status) {
    ...
    if (mac_addr) {
        Serial.printf("ESP-NOW sent %s -> %02X:%02X:%02X:%02X:%02X:%02X\n",
                      status == ESP_NOW_SEND_SUCCESS ? "OK" : "FAIL",
                      mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
    } else {
        Serial.println(status == ESP_NOW_SEND_SUCCESS ? "ESP-NOW sent OK" : "ESP-NOW sent FAIL");
    }
    #endif
...
void applyEspNowConfig(const struct EspNowConfigFrame& cfg) {
    ...
    if (cfg.applyBrightness) {
        ...
        preferences.end();
        settleFlashAfterNvsWrite();
        debugPrintln("ESP-NOW config: brillo=" + String(currentBrightnessLevel) + "%");
    }
    if (cfg.applyAudioAlert) {
        ...
        preferences.end();
        settleFlashAfterNvsWrite();
        debugPrintln(String("ESP-NOW config: audioAlert=") + (connectionAudioAlertEnabled ? "true" : "false"));
    }
    if (cfg.applyWiFi) debugPrintln("ESP-NOW config: bloque WiFi ignorado (modo ESP-NOW puro)");
...
bool initEspNowBridge() {
    ...
    if (esp_wifi_set_channel(configuredEspNowChannel, WIFI_SECOND_CHAN_NONE) != ESP_OK) {
        debugPrintln("ESP-NOW no pudo fijar el canal configurado");
        return false;
    }
    ...
    if (!parseMacString(configuredGatewayMac.c_str(), espnowGatewayMac)) {
        debugPrintln("ESP-NOW MAC gateway configurada invalida");
        return false;
    }
    if (esp_now_init() != ESP_OK) {
        debugPrintln("ESP-NOW init fallo");
        return false;
    }
    ...
    if (!esp_now_is_peer_exist(espnowGatewayMac)) {
        if (esp_now_add_peer(&peerInfo) != ESP_OK) {
            debugPrintln("ESP-NOW add_peer gateway fallo");
            return false;
        }
    }
    ...
    debugPrintln("ESP-NOW bridge listo (relay=" + String(ESPNOW_RELAY_ENABLED) + ")");
    return true;
}
...
bool tryRegisterViaRelay() {
    ...
    // Envia el RELAY_REQ actual al candidato senalado por curIdx (agrega el peer ESP-NOW si hace falta)
...
bool sendRegisterEspNow() {
    ...
    if (esp_now_send(espnowGatewayMac, (const uint8_t*)&reg, sizeof(reg)) != ESP_OK) {
        debugPrintln("ESP-NOW register send fallo");
        espnowExpectedAckSeq = 0;
        return false;
    }
    unsigned long t0 = millis();
    while (millis() - t0 < ESPNOW_ACK_TIMEOUT_MS) {
        if (espnowAckReceived && espnowAckSeq == reg.seq) {
            ...
            debugPrintln(ok ? "Registro ESP-NOW confirmado" : "Registro ESP-NOW rechazado");
    ...
    debugPrintln("Timeout esperando ACK de registro ESP-NOW");
...
bool enqueueEspNowFrame(bool active, int consecutive, const char* call_type) {
    if (!espnowBridgeReady) { debugPrintln("ESP-NOW bridge no listo"); return false; }
...
bool updateCall(bool active, int consecutive = 0, const char* call_type = CALL_TYPE_NORMAL, bool updateTimestamp = true) {
    ...
    debugPrintln(String(active ? "ACTIVAR" : "FINALIZAR") + " llamada -> ESP-NOW queue");
...
void updateNetworkInfo() {
    ...
    if (ui_LabelIp)       lv_label_set_text(ui_LabelIp,       "ESP-NOW");
    if (ui_LabelMac)      lv_label_set_text(ui_LabelMac,      deviceId.c_str());
    if (ui_Labelssid)     lv_label_set_text(ui_Labelssid,     "ESP-NOW-MESH");
...
void handleRemoteCallTermination() {
    debugPrintln("LLAMADA FINALIZADA remotamente (ESP-NOW gateway)");
...
void processInsistence(const char* source = "") {
    ...
    if (ok) {
        ...
        debugPrintln("Error al insistir por ESP-NOW");
    }
}
...
bool startNewCall(const char* source = "", bool changeScreen = true) {
    ...
    if (ok) {
        ...
        debugPrintln("Error al iniciar llamada por ESP-NOW");
    }
    return ok;
}
...
void handleFinishButton(lv_event_t *e) {
    ...
    if (!isCalling) return;
    if (!espnowBridgeReady) { debugPrintln("ESP-NOW bridge no listo"); return; }
    ...
    if (ok) {
        ...
        debugPrintln("Llamada finalizada - enviado por ESP-NOW");
    } else { debugPrintln("Error al finalizar por ESP-NOW"); }
}
...
void setup() {
    ...
    boot_set_row_result(bootRowRestore, BR_OK, "ESP-NOW");
    ...
    boot_set_global_status("Modo ESP-NOW activo");
    boot_process_ui_tick();
    ...
    debugPrintln("Sistema ESP-NOW listo! DeviceId: " + deviceId);
