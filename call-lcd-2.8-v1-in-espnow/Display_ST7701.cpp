 ...
 * Display_ST7701.cpp
 *
 * Controlador para la pantalla LCD basada en el driver ST7701 para ESP32-S3.
 * Inicializa el bus SPI, configura la pantalla, controla el backlight y permite dibujar regiones.
 *
 * Funciones principales:
 *   - ST7701_WriteCommand / ST7701_WriteData: Envían comandos/datos al controlador ST7701 por SPI.
 *   - ST7701_CS_EN / ST7701_CS_Dis: Controlan el pin de chip select (CS) del display.
 *   - ST7701_Reset: Realiza un reset físico al display.
 *   - ST7701_Init: Inicializa el bus SPI y la secuencia de configuración del ST7701.
...
// Manejador del dispositivo SPI y del panel LCD
spi_device_handle_t SPI_handle = NULL;     
esp_lcd_panel_handle_t panel_handle = NULL;    
uint8_t LCD_Backlight = 100; // Valor de brillo inicial (0-100)

// Envía un comando al controlador ST7701 por SPI
...
// Envía un dato al controlador ST7701 por SPI
...
// Habilita el pin CS (chip select) del display
void ST7701_CS_EN(){
  Set_EXIO(EXIO_PIN3,Low);
  vTaskDelay(pdMS_TO_TICKS(10));
}
// Deshabilita el pin CS (chip select) del display
void ST7701_CS_Dis(){
  Set_EXIO(EXIO_PIN3,High);
  vTaskDelay(pdMS_TO_TICKS(10));
}
// Realiza un reset físico al display
void ST7701_Reset(){
...
// Inicializa el bus SPI y la secuencia de configuración del ST7701
void ST7701_Init()
{
  // Configura el bus SPI para la comunicación con el display
  spi_bus_config_t buscfg = {
    ...
    .quadwp_io_num = -1,
    .quadhd_io_num = -1,
    .max_transfer_sz = 64, // Tamaño máximo de transferencia
  };
  ...
  spi_device_interface_config_t devcfg = {
    ...
    .spics_io_num = -1,                     
    .queue_size = 1,            // Sin colas
  };
  spi_bus_add_device(SPI2_HOST, &devcfg, &SPI_handle);            

  ST7701_CS_EN(); // Habilita CS para iniciar configuración
  // Secuencia de inicialización del controlador ST7701 (comandos específicos del fabricante)
  ...
  //ST7701_WriteCommand(0xE9);ST7701_WriteData( 2);ST7701_WriteData(ST7701_WriteCommand(0x36);ST7701_WriteData(0x40);
  ...
  ST7701_CS_Dis(); // Deshabilita CS tras la configuración

  // Configuración del panel RGB (resolución, pines, buffers, etc.)
  ...
  esp_lcd_new_rgb_panel(&rgb_config, &panel_handle); // Crea el panel RGB
  // esp_lcd_rgb_panel_event_callbacks_t cbs = {
  //   .on_vsync = example_on_vsync_event,
  // };
  // esp_lcd_rgb_panel_register_event_callbacks(panel_handle, &cbs, &disp_drv);
  esp_lcd_panel_reset(panel_handle); // Resetea el panel
  esp_lcd_panel_init(panel_handle);  // Inicializa el panel
  esp_lcd_panel_disp_on_off(panel_handle, true); // Enciende el panel (salida RGB activa)
}

// Callback de ejemplo para evento de vsync (no se usa en este ejemplo)
...
// Inicializa el display, realiza reset y también inicializa el touch
void LCD_Init() {
  ST7701_Reset();
  ST7701_Init();
  Touch_Init();
...
// Dibuja una región rectangular en la pantalla con los datos de color proporcionados
void LCD_addWindow(uint16_t Xstart, uint16_t Ystart, uint16_t Xend, uint16_t Yend,uint8_t* color) {
  Xend = Xend + 1;      // El parámetro x_end no es inclusivo
  Yend = Yend + 1;      // El parámetro y_end no es inclusivo
  if (Xend > ESP_PANEL_LCD_WIDTH)
    Xend = ESP_PANEL_LCD_WIDTH;
  if (Yend > ESP_PANEL_LCD_HEIGHT)
    Yend = ESP_PANEL_LCD_HEIGHT;

  esp_lcd_panel_draw_bitmap(panel_handle, Xstart, Ystart, Xend, Yend, color); // Dibuja el bitmap en la región
}


// Inicializa el control PWM para el backlight y lo ajusta al valor inicial
void Backlight_Init()
{
  // API disponible en tu core (ledcAttach con pin directamente)
  ledcAttach(LCD_Backlight_PIN, Frequency, Resolution);
  ledcWrite(LCD_Backlight_PIN, Dutyfactor);
  Set_Backlight(LCD_Backlight);      // 0~100                 
}

// Ajusta el brillo del backlight (0-100)
