...
void sendRegister() {
  ...
  esp_now_send(gatewayMac, reinterpret_cast<const uint8_t*>(&frame), sizeof(frame)); lastRegisterMs = millis();
#if ENABLE_SERIAL_DEBUG
...
void sendHeartbeat() {
  ...
  esp_now_send(BROADCAST_MAC, reinterpret_cast<const uint8_t*>(&presence), sizeof(presence)); lastHeartbeatMs = millis();
#if ENABLE_SERIAL_DEBUG
...
void requestSnapshot() {
  ...
  esp_now_send(gatewayMac, reinterpret_cast<const uint8_t*>(&frame), sizeof(frame)); lastSnapshotRequestMs = millis();
#if ENABLE_SERIAL_DEBUG
...
void setup() {
  ...
  if (!espNowReady) { Serial.println("FATAL: ESP-NOW initialization failed; outputs remain OFF"); return; }
